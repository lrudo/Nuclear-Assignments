...
This project allows us to solve the Schrodinger equation via the Shooting method while using the Woods-Saxon potential with angular momentum, ell, 0.

Once compiled, and the `./schrodinger`, `input_nuc.txt`, and `input_n_params.txt` executable run from the terminal, the resulting parameter values for mass, r0 (radius), Ea (energy), ka (momentum), ell (angular momentum l), x0 (initial value of x), nucA (atomic mass), nucZ (atomic number), nmax forward, and nmax backward (maximum number of integer steps to be taken) are saved to `261023938_CA_5_Woods_ell_0_schrodinger_params.dat`. As well, the data for the bound state error, the values of Et and Eb for the given energy range, and the forwards and backwards wavefunctions are saved to `261023938_CA_5_Woods_ell_0_bound_state_err.dat`, `261023938_CA_5_Woods_ell_0_schrodinger_data.dat`, `261023938_CA_5_Woods_ell_0_yf.dat`, and `261023938_CA_5_Woods_ell_0_yb.dat` respectively.

Instructions for changing the values for Et_min and Et_max to find the first 4 energy levels are included below under Usage, as well as in the `Usage.md` file.
...
- Nuc_261023938_CA_5_Woods_ell_0_yb.dat: The data for the backwards wave function for Et = 1.126115e-01
- Nuc_261023938_CA_5_Woods_ell_0_yf.dat: The data for the forwards wave function for Et = 1.126115e-01
- Nuc_261023938_CA_5_Woods_ell_0_bound_state_err.dat: Results from calculating the bound state error for ell_0 when Et_min=0.1 and Et_max=0.9999 .
- Nuc_261023938_CA_5_Woods_ell_0_schrodinger_data.dat: Contains the resulting Et and Eb values for the first 4 energy levels.

- Nuc_261023938_CA_5_Woods_ell_0_schrodinger_params.dat: Result file from `main_schrodinger.c` that displays the parameter values used.
- input_nuc.txt: Contains the values for mass, the energy unit eV, the angular momentum ell, NucA, and NucZ.
- input_n_params.txt: Contains the values for nmax forward and nmax backward.
- Woods_Saxon_ell_0_Bound_State_Error.png: Plots the error with respect to Et for l=0.
- Woods_Wavefunction_Lowest_Et_ell_0.png: Plots the forwards and backwards wavefunction for Et = 1.126115e-01 when l=0.


## Usage

Usage is described in `Usage.md`. However, instructions for calculating the first 4 energy levels are included below as well.
