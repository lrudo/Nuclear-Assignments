...
// Creates a '.dat' file for recording parameter values. Reads in the parameter values from PARAM_DATA, and converts the mass, energy, and momentum back into the eV unit. The values of all of the parameters, and their units, are then written to the '.dat' file.
{
  ...
  output = fopen("261023938_CA_5_Woods_ell_1_schrodinger_params.dat", "w"); // Creates DAT file for witing param values to
...
double *yf, double *yb)
{
  ...
  int n;
  double x;
  output = fopen("261023938_CA_5_Woods_ell_1_schrodinger_data.dat", "w");
  Et = D_Params.Et;
  ...
  output = fopen("261023938_CA_5_Woods_ell_1_yf.dat", "w");
  ...
  output = fopen("261023938_CA_5_Woods_ell_1_yb.dat","w");
